...
#   • Compact header: Help strip toggles into header via 'h' (no data refresh)
...
to_epoch() {
	...
	if [[ "$ts" =~ ^([0-9]{2})/([0-9]{2})/([0-9]{2})/([0-9]{2}):([0-9]{2}):([0-9]{2})$ ]]; then
		...
		# force base-10 (strip leading zeros safely)
		MM=$((10#$MM))
		DD=$((10#$DD))
		hh=$((10#$hh))
		mm=$((10#$mm))
...
while true; do
	...
	# Reserve two lines under title for: help strip (row 1) and refresh line (row 2)
	echo
	echo
	# Draw header strips now
	draw_help_line
	update_refresh_line

	# Optional CSV line (printed in body area; header strips are absolute-positioned)
	[[ -n "$LOG_FILE" ]] && echo "Logging CSV to: ${CLR_DIM}${LOG_FILE}${CLR_RESET}"

	# Small legend if sparklines enabled
	if ((SPARK_ON == 1)); then
