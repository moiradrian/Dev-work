...
if tput colors >/dev/null 2>&1; then
  ...
  CLR_GREEN=""; CLR_RED=""; CLR_YELLOW=""; CLR_DIM=""; CLR_BOLD=""; CLR_RESET=""
fi
...
# --- UI helper: live update of the "Refresh every ..." line (no data refresh) ---
update_refresh_line() {
  # Save cursor, jump to the refresh line, clear it, print new text, restore cursor.
  ...
  for g in LOCAL CLOUD GRAND; do
    ...
    if [[ -n "$pstart" && "$g" != "GRAND" ]]; then
      ...
      if [[ "$g" == "LOCAL" ]]; then
        ...
        if   [[ -z "$prev_val" ]]; then tok="D"
        elif (( spark_val > prev_val )); then tok="G"
        elif (( spark_val < prev_val )); then tok="R"
        else tok="D"; fi
  ...
  if (( PAUSED )); then
    while true; do
      if read -r -n1 -t 0.1 key; then
        case "$key" in
          ...
          +)   INTERVAL=$(( INTERVAL - INTERVAL_STEP )); (( INTERVAL < INTERVAL_MIN )) && INTERVAL="$INTERVAL_MIN"
               update_refresh_line; continue ;;
          -)   INTERVAL=$(( INTERVAL + INTERVAL_STEP )); (( INTERVAL > INTERVAL_MAX )) && INTERVAL="$INTERVAL_MAX"
               update_refresh_line; continue ;;
          b|B) base[LOCAL]="${curr[LOCAL]}"; base[CLOUD]="${curr[CLOUD]}"; base[GRAND]="${curr[GRAND]}";
               ...
               break ;;
          h|H) SHOW_HELP=$((1-SHOW_HELP)); break ;;
    ...
    while (( SECONDS < end )); do
      if read -r -n1 -t 0.1 key; then
        case "$key" in
          +)   INTERVAL=$(( INTERVAL - INTERVAL_STEP )); (( INTERVAL < INTERVAL_MIN )) && INTERVAL="$INTERVAL_MIN"
               end=$(( SECONDS + INTERVAL )); update_refresh_line; continue ;;
          -)   INTERVAL=$(( INTERVAL + INTERVAL_STEP )); (( INTERVAL > INTERVAL_MAX )) && INTERVAL="$INTERVAL_MAX"
               end=$(( SECONDS + INTERVAL )); update_refresh_line; continue ;;
          [rR]) break ;;           # manual refresh
          p|P) PAUSED=1; break ;;
