...
# NEW: color by instantaneous change (with deadband)
LAST_INST_LOCAL=-1
LAST_INST_CLOUD=-1
COLOR_DEADBAND_PCT="${COLOR_DEADBAND_PCT:-2}"        # % of previous inst (rounded)
...
while [[ $# -gt 0 ]]; do
	case "$1" in
	--csv) CSV_MODE=1 ;;
	--csv=*) CSV_MODE=1; LOG_FILE="${1#*=}" ;;
	--interval|-i) INTERVAL="${2:-3600}"; shift ;;
	--col|--column) RECLAIMED_COL="${2:-13}"; shift ;;
	--skip) SKIP_SUMMARY_LINES="${2:-12}"; shift ;;
	--ts-col) TS_COL="${2:-4}"; shift ;;
...
if tput colors >/dev/null 2>&1; then
	...
	CLR_GREEN=""; CLR_RED=""; CLR_YELLOW=""; CLR_DIM=""; CLR_BOLD=""; CLR_REV=""; CLR_RESET=""
fi
...
hr() {
	...
	if command -v numfmt >/dev/null 2>&1; then
		...
		while ((val >= 1024 && i < ${#units[@]} - 1)); do val=$(((val+512)/1024)); ((i++)); done
...
fmt_delta() {
	local d="${1:-0}"
	if   ((d>0)); then printf "%s+%s%s" "$CLR_GREEN" "$(hr "$d")" "$CLR_RESET"
	elif ((d<0)); then local ad=$((-d)); printf "%s-%s%s" "$CLR_RED" "$(hr "$ad")" "$CLR_RESET"
	else               printf "%s+0 B%s" "$CLR_DIM" "$CLR_RESET"
	fi
}
...
rate_bph() { local bytes="${1:-0}" secs="${2:-0}"; ((secs<=0)) && { echo 0; return; }; echo $(((bytes*3600)/secs)); }
rate_hr()  { local bytes="${1:-0}" secs="${2:-0}" r; r="$(rate_bph "$bytes" "$secs")"; ((r==0)) && { printf "n/a"; return; }; printf "%s/h" "$(hr "$r")"; }
...
to_epoch() {
	...
	if [[ "$ts" =~ ^([0-9]{2})/([0-9]{2})/([0-9]{2})/([0-9]{2}):([0-9]{2}):([0-9]{2})$ ]]; then
		...
		MM=$((10#$MM)); DD=$((10#$DD)); hh=$((10#$hh)); mm=$((10#$mm)); ss=$((10#$ss))
		local YYd=$((10#$YY)) YYYY; if ((YYd>=70)); then YYYY=$((1900+YYd)); else YYYY=$((2000+YYd)); fi
		norm=$(printf "%04d-%02d-%02d %02d:%02d:%02d" "$YYYY" "$MM" "$DD" "$hh" "$mm" "$ss")
		date -d "$norm" +%s 2>/dev/null || gdate -d "$norm" +%s 2>/dev/null || echo ""; return
	fi
	if [[ "$ts" =~ ^([0-9]{2})/([0-9]{2})/([0-9]{4})/([0-9]{2}):([0-9]{2}):([0-9]{2})$ ]]; then
		local MM="${BASH_REMATCH[1]}" DD="${BASH_REMATCH[2]}" YYYY="${BASH_REMATCH[3]}"
		local hh="${BASH_REMATCH[4]}" mm="${BASH_REMATCH[5]}" ss="${BASH_REMATCH[6]}"
		MM=$((10#$MM)); DD=$((10#$DD)); YYYY=$((10#$YYYY)); hh=$((10#$hh)); mm=$((10#$mm)); ss=$((10#$ss))
		norm=$(printf "%04d-%02d-%02d %02d:%02d:%02d" "$YYYY" "$MM" "$DD" "$hh" "$mm" "$ss")
		date -d "$norm" +%s 2>/dev/null || gdate -d "$norm" +%s 2>/dev/null || echo ""; return
	fi
...
spark() {
	...
	[[ -z "$vals_str" ]] && { echo " "; return; }
	local vals=($vals_str) toks=($toks_str) out=""
	local n=${#vals[@]}; ((n==0)) && { echo " "; return; }
	local min="${vals[0]}" max="${vals[0]}"
	for v in "${vals[@]}"; do ((v<min)) && min="$v"; ((v>max)) && max="$v"; done
	local range=$((max-min)) blocks=(▁ ▂ ▃ ▄ ▅ ▆ ▇ █)
	for i in "${!vals[@]}"; do
		local v="${vals[$i]}" idx
		if ((range==0)); then idx=3
		else idx=$(( ((v-min)*(${#blocks[@]}-1))/range )); ((idx<0)) && idx=0; ((idx>${#blocks[@]}-1)) && idx=${#blocks[@]}-1; fi
...
draw_status_line() {
	...
	tput el 2>/dev/null || true
	local badge; if ((PAUSED)); then badge="${CLR_YELLOW}[PAUSED]${CLR_RESET}"; else badge="${CLR_GREEN}[RUNNING]${CLR_RESET}"; fi
...
pulse_updated_ts() {
	...
	tput sc 2>/dev/null || true
	tput cup 0 0 2>/dev/null || true; tput el 2>/dev/null || true; echo -n "$line_pulse"
	sleep "$(awk -v ms="$HEARTBEAT_MS" 'BEGIN{printf "%.3f", ms/1000.0}')"
	tput cup 0 0 2>/dev/null || true; tput el 2>/dev/null || true; echo -n "$line_norm"
	tput rc 2>/dev/null || true
}

LAST_COUNTDOWN_STR=""; LAST_COUNTDOWN_TS=0

clear_footer_area() {
	local rows cols row_bar row_count
	rows="$(tput lines 2>/dev/null || echo 24)"
	cols="$(tput cols 2>/dev/null || echo 120)"
	row_count=$((rows - COUNTDOWN_OFFSET)); row_bar=$((row_count - 1))
	tput sc 2>/dev/null || true
	if ((row_bar >= 0)); then tput cup "$row_bar" 0 2>/dev/null || true; printf "%-*s" "$cols" ""; fi
	((row_count < 0)) && row_count=0
	tput cup "$row_count" 0 2>/dev/null || true; printf "%-*s" "$cols" ""
	tput rc 2>/dev/null || true
}
...
draw_footer_bar() {
	...
	rows="$(tput lines 2>/dev/null || echo 24)"
	cols="$(tput cols 2>/dev/null || echo 120)"
	row=$((rows - COUNTDOWN_OFFSET - 1)); ((row < 0)) && row=0
	ch='-'; ((UNICODE_BORDERS == 1)) && ch='─'
	tput sc 2>/dev/null || true
...
draw_countdown() {
	...
	if ((SHOW_COUNTDOWN == 0)); then clear_footer_area; LAST_COUNTDOWN_STR=""; return 0; fi
	local now="$SECONDS"; ((now - LAST_COUNTDOWN_TS < COUNTDOWN_THROTTLE_SEC)) && return 0
	LAST_COUNTDOWN_TS="$now"
	local cols rows row prefix time_str color line
	rows="$(tput lines 2>/dev/null || echo 24)"; cols="$(tput cols 2>/dev/null || echo 120)"
	row=$((rows - COUNTDOWN_OFFSET)); ((row < 0)) && row=0
	if ((paused)); then prefix="(PAUSED) Press 'p' to resume. Keys: + - r b h c f q"; time_str=""; color="$CLR_YELLOW"
	else
		prefix="(Press '+' / '-' / 'r' / 'p' / 'b' / 'h' / 'c' / 'f' / 'q')  Next refresh in "
		time_str="$(fmt_secs_long "$rem")"
		if   ((rem <= COUNTDOWN_CRIT)); then color="$CLR_RED"
		elif ((rem <= COUNTDOWN_WARN)); then color="$CLR_YELLOW"
	...
	draw_footer_bar
	tput sc 2>/dev/null || true; tput cup "$row" 0 2>/dev/null || true; printf "%-*s" "$cols" "$line"; tput rc 2>/dev/null || true
}
...
while true; do
	...
	# Initialize baseline
	if ((BASE_INIT == 0)); then
		base[LOCAL]="${curr[LOCAL]}"; base[CLOUD]="${curr[CLOUD]}"; base[GRAND]="${curr[GRAND]}"
		prev[LOCAL]="${curr[LOCAL]}"; prev[CLOUD]="${curr[CLOUD]}"; prev[GRAND]="${curr[GRAND]}"
		BASE_INIT=1
	...
	for g in LOCAL CLOUD GRAND; do
		...
		if [[ -n "$pstart" && "$g" != "GRAND" ]]; then
			...
			if [[ -n "$ps_epoch" ]]; then
				...
				elapsed=0; rate_text="n/a"; rate_int=0
			fi
			...
			if [[ "$g" == "LOCAL" && ${#rate_local_vals[@]} -eq 0 && "$rate_int" -ge 0 ]]; then
				rate_local_vals+=("$rate_int"); rate_local_colors+=("D")
				LAST_EPOCH_LOCAL="$now_epoch"; skip_inst=1
			elif [[ "$g" == "CLOUD" && ${#rate_cloud_vals[@]} -eq 0 && "$rate_int" -ge 0 ]]; then
				rate_cloud_vals+=("$rate_int"); rate_cloud_colors+=("D")
				LAST_EPOCH_CLOUD="$now_epoch"; skip_inst=1
			fi

			# Instantaneous per-interval rate
			step_bytes=$((total - prev[$g])); ((step_bytes < 0)) && step_bytes=0
			if [[ "$g" == "LOCAL" ]]; then last_ep="$LAST_EPOCH_LOCAL"; else last_ep="$LAST_EPOCH_CLOUD"; fi
			if [[ -z "$last_ep" || "$last_ep" -le 0 ]]; then elapsed_step=$((INTERVAL>0 ? INTERVAL : 1))
			else elapsed_step=$((now_epoch - last_ep)); ((elapsed_step <= 0)) && elapsed_step=1; fi
			...
			# --- NEW: color by instantaneous change (with deadband) ---
			if [[ "$g" == "LOCAL" ]]; then prev_inst="$LAST_INST_LOCAL"; else prev_inst="$LAST_INST_CLOUD"; fi

			tok="D"
			if (( prev_inst >= 0 )); then
				# relative deadband threshold (rounded) + absolute floor
				rel_thresh=$(( (prev_inst * COLOR_DEADBAND_PCT + 50) / 100 ))
				(( rel_thresh < COLOR_DEADBAND_ABS_BPH )) && rel_thresh="$COLOR_DEADBAND_ABS_BPH"
				diff=$(( inst_bph - prev_inst ))
				if   (( diff >  rel_thresh )); then tok="G"
				elif (( diff < -rel_thresh )); then tok="R"
				else tok="D"; fi
			fi

			# Append (height=EMA, color=tok)
			if [[ "$g" == "LOCAL" ]]; then
				if (( skip_inst == 0 )); then
					rate_local_vals+=("$spark_val"); rate_local_colors+=("$tok")
				fi
				...
				LAST_INST_LOCAL="$inst_bph"
			else
				if (( skip_inst == 0 )); then
					rate_cloud_vals+=("$spark_val"); rate_cloud_colors+=("$tok")
				fi
		...
		if ((SPARK_ON == 1)); then
			...
			if ((SPARK_WIDTH > 0)); then points=$SPARK_WIDTH
			else
				points=$SPARK_POINTS
			...
			# render even a single point so seed is visible
			if [[ "$g" == "LOCAL" && ${#rate_local_vals[@]} -gt 0 ]]; then
				draw_vals=("${rate_local_vals[@]}"); draw_toks=("${rate_local_colors[@]}")
				(( ${#draw_vals[@]} > points )) && {
			...
			if [[ "$g" == "CLOUD" && ${#rate_cloud_vals[@]} -gt 0 ]]; then
				draw_vals=("${rate_cloud_vals[@]}"); draw_toks=("${rate_cloud_colors[@]}")
				(( ${#draw_vals[@]} > points )) && {
					draw_vals=("${draw_vals[@]: -$points}")
					draw_toks=("${draw_toks[@]: -$points}")
				}
	...
	if ((PAUSED)); then
		while true; do
			...
			if read -r -n1 -t 0.2 key; then
				case "$key" in
				p|P) PAUSED=0; draw_status_line; break ;;
				[rR]) break ;;
				+)   INTERVAL=$((INTERVAL - INTERVAL_STEP)); ((INTERVAL < INTERVAL_MIN)) && INTERVAL="$INTERVAL_MIN"; update_refresh_line; continue ;;
				-)   INTERVAL=$((INTERVAL + INTERVAL_STEP)); ((INTERVAL > INTERVAL_MAX)) && INTERVAL="$INTERVAL_MAX"; update_refresh_line; continue ;;
				b|B) base[LOCAL]="${curr[LOCAL]}"; base[CLOUD]="${curr[CLOUD]}"; base[GRAND]="${curr[GRAND]}";
				     prev[LOCAL]="${curr[LOCAL]}"; prev[CLOUD]="${curr[CLOUD]}"; prev[GRAND]="${curr[GRAND]}";
				     BASELINE_TIME="$(date '+%Y-%m-%d %H:%M:%S') (manual)";
				     STATUS_MSG="${CLR_GREEN}Baseline set at ${BASELINE_TIME}${CLR_RESET}"; draw_status_line; break ;;
				h|H) SHOW_HELP=$((1-SHOW_HELP)); draw_help_line; continue ;;
				c|C) SHOW_COUNTDOWN=$((1-SHOW_COUNTDOWN)); LAST_COUNTDOWN_STR=""; clear_footer_area; continue ;;
				f|F) SHOW_FOOTER_BAR=$((1-SHOW_FOOTER_BAR)); LAST_COUNTDOWN_STR=""; clear_footer_area; continue ;;
		...
		while ((SECONDS < end)); do
			...
			if read -r -n1 -t 0.2 key; then
				case "$key" in
				+)   INTERVAL=$((INTERVAL - INTERVAL_STEP)); ((INTERVAL < INTERVAL_MIN)) && INTERVAL="$INTERVAL_MIN"; end=$((SECONDS + INTERVAL)); update_refresh_line; continue ;;
				-)   INTERVAL=$((INTERVAL + INTERVAL_STEP)); ((INTERVAL > INTERVAL_MAX)) && INTERVAL="$INTERVAL_MAX"; end=$((SECONDS + INTERVAL)); update_refresh_line; continue ;;
				[rR]) break ;;
				p|P) PAUSED=1; draw_status_line; break ;;
				b|B) base[LOCAL]="${curr[LOCAL]}"; base[CLOUD]="${curr[CLOUD]}"; base[GRAND]="${curr[GRAND]}";
				     prev[LOCAL]="${curr[LOCAL]}"; prev[CLOUD]="${curr[CLOUD]}"; prev[Grand]="${curr[GRAND]}";
				     BASELINE_TIME="$(date '+%Y-%m-%d %H:%M:%S') (manual)";
				     STATUS_MSG="${CLR_GREEN}Baseline set at ${BASELINE_TIME}${CLR_RESET}"; draw_status_line; break ;;
