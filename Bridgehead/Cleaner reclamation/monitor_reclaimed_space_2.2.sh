...
#   • Live bottom-line countdown timer
...
# Live countdown position & repaint throttle
COUNTDOWN_OFFSET="${COUNTDOWN_OFFSET:-2}"             # 1 = very last row; 2 = one row above bottom, etc.
COUNTDOWN_THROTTLE_SEC="${COUNTDOWN_THROTTLE_SEC:-1}" # repaint at most once per N seconds
LAST_COUNTDOWN_STR=""
LAST_COUNTDOWN_TS=0
...
draw_countdown() {
	...
	local msg
	if ((paused)); then
		msg="(PAUSED) Press 'p' to resume. Keys: + - r b h q"
	else
		msg="(Press '+' / '-' / 'r' / 'p' / 'b' / 'h' / 'q')  Next refresh in $(fmt_secs_long "$rem")"
	fi

	# if unchanged, skip draw
	if [[ "$msg" == "$LAST_COUNTDOWN_STR" ]]; then
		return 0
	fi
	LAST_COUNTDOWN_STR="$msg"

	# compute target row from bottom
	local rows="$(tput lines 2>/dev/null || echo 24)"
	local row=$((rows - COUNTDOWN_OFFSET))
	((row < 0)) && row=0

	# draw without clearing the whole line (avoid flicker): print msg and pad to EOL
	tput sc 2>/dev/null || true
	tput cup "$row" 0 2>/dev/null || true
	# pad to end to overwrite remnants; use terminal columns to size the pad
