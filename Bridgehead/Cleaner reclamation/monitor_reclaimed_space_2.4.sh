...
FLASH_MS="${FLASH_MS:-100}" # duration in ms (approx)
...
clear_footer_area() {
	# Clears both the footer bar row and the countdown row
...
draw_footer_bar() {
	...
	((row < 0)) && row=0
	ch='-'
	if ((UNICODE_BORDERS == 1)); then ch='─'; fi
	tput sc 2>/dev/null || true
	tput cup "$row" 0 2>/dev/null || true
	# Draw a full-width bar using the chosen character
	printf "%*s" "$cols" "" | tr ' ' "$ch"
	tput rc 2>/dev/null || true
}
...
draw_countdown() {
	...
	# If hidden, ensure footer area is blank and bail
	...
	# Build final line with only the time colored
	if [[ -n "$time_str" ]]; then
		line="${prefix}${color}${time_str}${CLR_RESET}"
	else
		line="${color}${prefix}${CLR_RESET}"
	fi

	# if unchanged, skip draw
	if [[ "$line" == "$LAST_COUNTDOWN_STR" ]]; then
		return 0
	fi
	...
	# draw countdown line (pad to EOL)
	tput sc 2>/dev/null || true
	tput cup "$row" 0 2>/dev/null || true
	printf "%-*s" "$cols" "$line"
	tput rc 2>/dev/null || true
...
while true; do
	...
	if ((SHOW_HELP)); then
		...
		echo "Countdown: $([[ $SHOW_COUNTDOWN -eq 1 ]] && echo ON || echo OFF), Footer bar: $([[ $SHOW_FOOTER_BAR -eq 1 ]] && echo ON || echo OFF), Borders: $([[ $UNICODE_BORDERS -eq 1 ]] && echo Unicode || echo ASCII)"
		echo "--------------------------------------------------------------------------------"
	fi
	...
	for g in LOCAL CLOUD GRAND; do
		...
		if [[ -n "$pstart" && "$g" != "GRAND" ]]; then
			...
			# Persistent color token (vs previous)
			if [[ "$g" == "LOCAL" ]]; then
				if ((${#rate_local_vals[@]} == 0)); then
					prev_val=""
				else prev_val="${rate_local_vals[$((${#rate_local_vals[@]} - 1))]}"; fi
				if [[ -z "$prev_val" ]]; then
					tok="D"
				...
				if ((${#rate_cloud_vals[@]} == 0)); then
					prev_val=""
				else prev_val="${rate_cloud_vals[$((${#rate_cloud_vals[@]} - 1))]}"; fi
				if [[ -z "$prev_val" ]]; then
					tok="D"
				elif ((spark_val > prev_val)); then
					tok="G"
