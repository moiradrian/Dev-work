...
SHOW_SPARK_LEGEND="${SHOW_SPARK_LEGEND:-1}" # NEW: show mini legend under sparks
...
STATUS_LINE_ROW=1  # NEW: status strip row (below title)
HELP_LINE_ROW=2    # NEW: help strip row (below status)
REFRESH_LINE_ROW=3 # moved down one row
...
FLASH_ON_REFRESH="${FLASH_ON_REFRESH:-0}" # legacy whole-line flash (off now)
FLASH_MS="${FLASH_MS:-100}"               # ms (approx)
HEARTBEAT_MS="${HEARTBEAT_MS:-120}"       # NEW: pulse duration for updated: timestamp
...
draw_status_line() { # NEW: compact status strip
	tput sc 2>/dev/null || true
	tput cup "$STATUS_LINE_ROW" 0 2>/dev/null || true
	tput el 2>/dev/null || true
	local state badge
	if ((PAUSED)); then
		badge="${CLR_YELLOW}[PAUSED]${CLR_RESET}"
	else
		badge="${CLR_GREEN}[RUNNING]${CLR_RESET}"
	fi
	if [[ -n "$BASELINE_TIME" ]]; then
		echo -e "$badge  ${CLR_DIM}Baseline:${CLR_RESET} $BASELINE_TIME"
	else
		echo -e "$badge"
	fi
	tput rc 2>/dev/null || true
...
pulse_updated_ts() { # NEW: heartbeat — briefly highlight the updated timestamp
	local now="$(date '+%Y-%m-%d %H:%M:%S')"
	# line text (normal)
	local line_norm
	line_norm=$(printf "%sReclaimed totals%s (started: %s | updated: %s)" \
		"$CLR_BOLD" "$CLR_RESET" "$START_TIME" "$now")
	# line with updated highlighted
	local line_pulse
	line_pulse=$(printf "%sReclaimed totals%s (started: %s | updated: %s%s%s)" \
		"$CLR_BOLD" "$CLR_RESET" "$START_TIME" "$CLR_YELLOW" "$now" "$CLR_RESET")

	tput sc 2>/dev/null || true
	tput cup 0 0 2>/dev/null || true
	tput el 2>/dev/null || true
	echo -n "$line_pulse"
	# sleep ~HEARTBEAT_MS (portable)
...
draw_countdown() {
	...
	else line="${color}${prefix}${CLR_RESET}"; fi

	if [[ "$line" == "$LAST_COUNTDOWN_STR" ]]; then return 0; fi
