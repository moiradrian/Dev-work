...
# UI: which row shows "Refresh every ..." (0-indexed after clear)
REFRESH_LINE_ROW=2
...
SHOW_FOOTER_BAR="${SHOW_FOOTER_BAR:-1}" # draw a '─' bar above countdown
...
# Header flash on refresh
FLASH_ON_REFRESH="${FLASH_ON_REFRESH:-1}"
FLASH_MS="${FLASH_MS:-100}" # duration in milliseconds (approx)
...
Keys:
  ...
  c toggle countdown   f toggle footer bar
EOF
		exit 0
		;;
	*)
...
spark() {
	...
	# min–max scaling; flat => mid-level bar
	...
	for i in "${!vals[@]}"; do
		...
		local idx
		if ((range == 0)); then
			idx=3 # mid-level for flat window
		else
		...
		# choose color from token array (default dim)
...
draw_countdown() {
	# args: remaining_secs, paused_flag
	local rem="${1:-0}" paused="${2:-0}"
	((SHOW_COUNTDOWN == 0)) && return 0

	# throttle repaints
	local now="$SECONDS"
	if ((now - LAST_COUNTDOWN_TS < COUNTDOWN_THROTTLE_SEC)); then
		return 0
	fi
	LAST_COUNTDOWN_TS="$now"

	local cols rows row msg color=""
	rows="$(tput lines 2>/dev/null || echo 24)"
	cols="$(tput cols 2>/dev/null || echo 120)"
	...
	if ((paused)); then
		msg="(PAUSED) Press 'p' to resume. Keys: + - r b h c f q"
		color="$CLR_YELLOW"
	else
		msg="(Press '+' / '-' / 'r' / 'p' / 'b' / 'h' / 'c' / 'f' / 'q')  Next refresh in $(fmt_secs_long "$rem")"
	...
	# if unchanged, skip draw
	if [[ "${color}${msg}${CLR_RESET}" == "$LAST_COUNTDOWN_STR" ]]; then
		return 0
	fi
	LAST_COUNTDOWN_STR="${color}${msg}${CLR_RESET}"

	# optional footer bar just above countdown
	draw_footer_bar

	# draw countdown line (pad to EOL to overwrite remnants)
	tput sc 2>/dev/null || true
	tput cup "$row" 0 2>/dev/null || true
	printf "%s%-*s%s" "$color" "$cols" "$msg" "$CLR_RESET"
	tput rc 2>/dev/null || true
}

flash_header() {
	((FLASH_ON_REFRESH == 0)) && return 0
	# Try terminal flash; if unsupported, do a quick reverse-video pulse on the title line
	if tput flash >/dev/null 2>&1; then
		tput flash
	else
		tput sc 2>/dev/null || true
		tput cup 0 0 2>/dev/null || true
		printf "%s" "$CLR_REV"
		# Reprint the first header line in reverse video
		printf "Reclaimed totals (started: %s | updated: %s)" "$START_TIME" "$(date '+%Y-%m-%d %H:%M:%S')"
		printf "%s" "$CLR_RESET"
		sleep "$(awk -v ms="$FLASH_MS" 'BEGIN{printf "%.3f", ms/1000.0}')"
		# Redraw the normal line
		tput cup 0 0 2>/dev/null || true
		tput el 2>/dev/null || true
		printf "%sReclaimed totals%s (started: %s | updated: %s)" "$CLR_BOLD" "$CLR_RESET" "$START_TIME" "$(date '+%Y-%m-%d %H:%M:%S')"
		tput rc 2>/dev/null || true
	fi
}
...
while true; do
	...
	echo "Controls: '+' faster (-5m), '-' slower (+5m), 'r' refresh, 'p' pause, 'b' baseline, 'h' help, 'c' countdown, 'f' footer, 'q' quit"
	echo "Refresh every $(fmt_secs "$INTERVAL")"
	...
	# Help overlay
	if ((SHOW_HELP)); then
		echo "${CLR_BOLD}Keys:${CLR_RESET} + faster  - slower  r refresh  p pause  b baseline  h help  c countdown  f footer  q quit"
		echo "Alerts when rate ≥ $(hr "$ALERT_RATE_BPH")/h  |  Sparkline window: ${SPARK_POINTS} samples  |  Spark: $([[ $SPARK_ON -eq 1 ]] && echo ON || echo OFF)"
		echo "Countdown: $([[ $SHOW_COUNTDOWN -eq 1 ]] && echo ON || echo OFF), Footer bar: $([[ $SHOW_FOOTER_BAR -eq 1 ]] && echo ON || echo OFF)"
		echo "--------------------------------------------------------------------------------"
	fi
	...
	for g in LOCAL CLOUD GRAND; do
		...
		# second line: period start and average rate
		rate_text=""
		rate_int=0
		line2=""
		if [[ -n "$pstart" && "$g" != "GRAND" ]]; then
			...
			# Ignore ultra-short samples unless bytes changed
			if ((elapsed_step < MIN_SAMPLE_SECS && step_bytes == 0)); then
				inst_bph=0
			else
				inst_bph=$(((step_bytes * 3600 + elapsed_step / 2) / elapsed_step))
				if ((step_bytes > 0 && inst_bph == 0)); then inst_bph=$MIN_VISIBLE_BPH; fi
			fi
			...
			# G = up, R = down, D = same
			if [[ "$g" == "LOCAL" ]]; then
				if ((${#rate_local_vals[@]} == 0)); then prev_val=""; else prev_val="${rate_local_vals[$((${#rate_local_vals[@]} - 1))]}"; fi
				if [[ -z "$prev_val" ]]; then
					tok="D"
				...
				if ((${#rate_cloud_vals[@]} == 0)); then prev_val=""; else prev_val="${rate_cloud_vals[$((${#rate_cloud_vals[@]} - 1))]}"; fi
				if [[ -z "$prev_val" ]]; then
					tok="D"
				elif ((spark_val > prev_val)); then
					tok="G"
	...
	if ((PAUSED)); then
		...
		# refresh pulse as soon as this draw is on-screen
		flash_header
