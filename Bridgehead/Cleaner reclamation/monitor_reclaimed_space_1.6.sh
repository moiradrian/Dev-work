...
# Features:
#   • Hourly default refresh; +/- 5m steps (5m..2h), manual refresh (r/space)
...
#   • Keys: + faster, - slower, r/space refresh, p pause, b baseline, h help, q quit
...
Keys:
  + faster (-5m)   - slower (+5m)   r/space refresh   p pause   b baseline   h help   q quit
EOF
      exit 0
      ;;
...
if tput colors >/dev/null 2>&1; then
  ...
  CLR_GREEN=""; CLR_RED=""; CLR_DIM=""; CLR_BOLD=""; CLR_RESET=""
fi
...
# Sparkline (bytes/hour), newest highlighted
spark() {
  [[ -z "${1-}" ]] && { echo " "; return; }
  ...
  for i in "${!vals[@]}"; do
    ...
    if (( i == last_index )); then
      out+="${CLR_GREEN}${blocks[$idx]}${CLR_RESET}"
    else
      out+="${CLR_DIM}${blocks[$idx]}${CLR_RESET}"
    fi
  done
  echo "$out"
}
    ...
    while true; do
      if read -r -n1 -t 0.1 key; then
        case "$key" in
          p|P) PAUSED=0; break ;;
          [rR]|" ") break ;;
          +)   INTERVAL=$(( INTERVAL - INTERVAL_STEP )); (( INTERVAL < INTERVAL_MIN )) && INTERVAL="$INTERVAL_MIN"; break ;;
          -)   INTERVAL=$(( INTERVAL + INTERVAL_STEP )); (( INTERVAL > INTERVAL_MAX )) && INTERVAL="$INTERVAL_MAX"; break ;;
          b|B) base[LOCAL]="${curr[LOCAL]}"; base[CLOUD]="${curr[CLOUD]}"; base[GRAND]="${curr[GRAND]}"; prev[LOCAL]="${curr[LOCAL]}"; prev[CLOUD]="${curr[CLOUD]}"; prev[GRAND]="${curr[GRAND]}"; ;;
