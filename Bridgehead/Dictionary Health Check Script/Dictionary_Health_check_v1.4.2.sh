...
# ------ Cleaner Status Summary (default mode) ------
if [[ -z "$MODE" ]]; then
    # get the full cleaner_adminstats once
    CLEANER_STATS=$(ctrlrpc -p 9911 show.cleaner_adminstats)

    # extract Local cleaner status (the word after “Status:” under “Local cleaner:”)
    LSTATUS=$(echo "$CLEANER_STATS" | awk '
    /^Local cleaner:/     { in_local=1; next }
    in_local && /^Status:/ { print $2; exit }
  ')

    # extract Cloud cleaner status (the word after “Status:” under “Cloud cleaner:”)
    CSTATUS=$(echo "$CLEANER_STATS" | awk '
    /^Cloud cleaner:/     { in_cloud=1; next }
    in_cloud && /^Status:/  { print $2; exit }
  ')

    echo
    echo "Local cleaner: $LSTATUS"
    echo "Cloud cleaner: $CSTATUS"
    echo
fi
