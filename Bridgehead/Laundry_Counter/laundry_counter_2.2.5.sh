...
FULL_REFRESH_INTERVAL=30 # seconds between full rescan to reconcile counts
...
last_full_refresh=$(date +%s)
	...
	while read -r watched_dir filename events; do
		...
		full_path="$parent_dir/$filename"
			...
			# If a new directory was created/added, refresh watches so files inside are tracked
