...
ALT_LAYOUT="false"
R3_JOURNAL_PATH=""
# Where refcounts live (source) vs where they must land (destination)
REFCNT_SRC_SUBPATH=".ocarina_hidden/refcnt" # source tail in repo (standard layout)
REFCNT_DST_SUBPATH="refcnt"                 # destination tail on SSD (ALWAYS 'refcnt')
...
# ---- Functions ----
usage() {
	echo "Usage: $0 [--dry-run] [--scan-only] [--copy-only] [--checksum-verify] [MOUNTPOINT]"
	echo
	...
	echo "  --copy-only         Copy refcnt data (with progress) but do NOT edit config or stop/start services"
...
detect_layout_once() {
	...
	declare -g REFCNT_SRC_SUBPATH=".ocarina_hidden/refcnt"
	declare -g REFCNT_DST_SUBPATH="refcnt"
	...
	if [[ -n "$has_refcnt_on_ssd" && -n "$has_tgt_is_r3" ]]; then
		...
		if [[ -n "$r3val" && -d "$r3val" ]]; then
			ALT_LAYOUT="true"
			REFCNT_SRC_SUBPATH="refcnt" # alt/R3 reads from <repo>/<N>/refcnt
			REFCNT_DST_SUBPATH="refcnt" # destination stays 'refcnt'
	...
	debug_log "detect_layout_once: SRC_SUBPATH=$REFCNT_SRC_SUBPATH DST_SUBPATH=$REFCNT_DST_SUBPATH"
...
get_repo_location() {
	...
	if [[ "$ALT_LAYOUT" == "true" ]]; then
		...
		debug_log "get_repo_location: ALT repo -> $R3_JOURNAL_PATH (SRC=$REFCNT_SRC_SUBPATH DST=$REFCNT_DST_SUBPATH)"
	...
	debug_log "get_repo_location: standard repo -> $repo (SRC=$REFCNT_SRC_SUBPATH DST=$REFCNT_DST_SUBPATH)"
	printf "%s" "$repo"
}
...
plan_copy_totals() {
	...
	for d in "$repo"/*; do
		...
		[[ "$base" =~ ^[0-9]+$ ]] || continue
		local refdir="$d/$REFCNT_SRC_SUBPATH"
		if [[ -d "$refdir" ]]; then
			local count
...
scan_refcnt_sizes() {
	...
	echo "Looking for integer dirs with '$REFCNT_SRC_SUBPATH'"
	echo
...
copy_one_refcnt() {
	...
	# Expect globals: REFCNT_SRC_SUBPATH (source tail) and REFCNT_DST_SUBPATH (destination tail)
	# If SRC is an integer dir (or its base) and SRC/$REFCNT_SRC_SUBPATH exists, fix SRC/DST.
	local tail="${SRC%/}"
	tail="${tail##*/}"                      # last path component of SRC
	local need_src_tail="${REFCNT_SRC_SUBPATH##*/}" # usually "refcnt"
	local need_dst_tail="${REFCNT_DST_SUBPATH##*/}" # always "refcnt"

	if [[ "$tail" != "$need_src_tail" && -d "$SRC/$REFCNT_SRC_SUBPATH" ]]; then
		debug_log "copy_one_refcnt: auto-appending REFCNT_SRC_SUBPATH to SRC and REFCNT_DST_SUBPATH to DST (layout fix)"
		SRC="$SRC/$REFCNT_SRC_SUBPATH"
		# Only append to DST if it doesn't already end with the destination tail
		local dst_tail="${DST%/}"
		dst_tail="${dst_tail##*/}"
		[[ "$dst_tail" == "$need_dst_tail" ]] || DST="$DST/$REFCNT_DST_SUBPATH"
	fi
	# --- end normalization ---
...
verify_one_refcnt() {
	...
	if [ "$DRY_RUN" = "true" ]; then
		...
		out="$(safe_rsync "${RSYNC_ARGS[@]}" "$SRC/" "$DST/" 2>&1 || true)"
			...
			files="$(echo "$out" | awk -F': ' '/^Number of files/ {gsub(/[^0-9]/,"",$2); print $2+0}')"
		: "${files:=0}"
...
cleanup_copy_only() {
	# Prompt to remove the copied data; if not, show size and re-prompt/exit
	if [[ -z "$MOUNTPOINT" ]]; then
		return
	fi
	local ssd_path="${MOUNTPOINT}/ssd"
	while true; do
		read -rp "Cleanup copied data at ${ssd_path}? [y/N]: " ans
		case "${ans,,}" in
		y|yes)
			echo "Removing ${ssd_path} ..."
			rm -rf -- "${ssd_path}"
			echo "Cleanup complete."
			SUMMARY+=("Cleanup: removed ${ssd_path}")
			return
			;;
		*)
			local sz_bytes sz_h
			if [[ -d "$ssd_path" ]]; then
				sz_bytes=$(du -sb "$ssd_path" 2>/dev/null | awk '{print $1}')
				: "${sz_bytes:=0}"
				sz_h=$(human_bytes "$sz_bytes")
			else
				sz_h="0B"
			fi
			echo "Consumed ${sz_h} on ${MOUNTPOINT}"
			read -rp "Are you sure? [y/N]: " ans2
			case "${ans2,,}" in
			y|yes)
				echo "Leaving data in place and exiting."
				SUMMARY+=("Cleanup skipped; data retained at ${ssd_path} (${sz_h})")
				return
				;;
			*)
				# Loop back to cleanup question
				continue
				;;
			esac
			;;
...
copy_all_refcnt() {
	...
	# ---- Per-tree copy so DST can differ from SRC tail ----
	# SRC:  <repo>/<N>/${REFCNT_SRC_SUBPATH}    (# layout-dependent; your script sets this)
	# DST:  <ssd>/<N>/refcnt                    (# ALWAYS this)
	local any_found=0
	local any_copied=0
	local -i total_counted_files=0
	local -i overall_copied=0
	local -i overall_planned="$planned_files"
	local status_width=40

	draw_overall() {
		local current_base="$1" files_in_src="$2"
		local pct=0
		if ((overall_planned > 0)); then
			pct=$((overall_copied * 100 / overall_planned))
		fi
		local filled=$((pct * status_width / 100))
		local filled_bar empty_bar bar
		filled_bar=$(printf '%*s' "$filled" '' | tr ' ' '#')
		empty_bar=$(printf '%*s' $((status_width - filled)) '' | tr ' ' '.')
		bar="[$filled_bar$empty_bar]"
		if [ -n "$current_base" ]; then
			printf "\rCopying %-8s (%'d files) Overall: %'d / %'d %s %3d%%" \
				"$current_base" "$files_in_src" "$overall_copied" "$overall_planned" "$bar" "$pct"
		else
			printf "\rOverall: %'d / %'d %s %3d%%" "$overall_copied" "$overall_planned" "$bar" "$pct"
		fi
	}
	...
	for d in "$repo"/*; do
		...
		local SRC="$d/$REFCNT_SRC_SUBPATH"
		local DST="$target_base/$base/$REFCNT_DST_SUBPATH"

		if [[ ! -d "$SRC" ]]; then
			echo "Note: Missing source refcnt for $base -> $SRC (skipped)"
			SUMMARY+=("✘ Skip (missing SRC): $SRC")
			continue
		fi

		# Create destination directory only in LIVE mode
		if [ "$DRY_RUN" != "true" ]; then
			mkdir -p "$DST"
		fi

		local -a RSYNC_ARGS
		local tmpfile files_this_dir
		local files_in_src

		if [ "$DRY_RUN" = "true" ]; then
			# DRY-RUN: estimate per-tree using rsync -n --stats
			IFS=$'\n' read -r -d '' -a RSYNC_ARGS < <(rsync_base_flags && printf '\0')
			RSYNC_ARGS+=(--stats -n)
			...
			# In dry-run, rsync reports 0 "transferred"; count planned files instead.
			files_this_dir="$(awk -F': ' '/^Number of files/ {gsub(/[^0-9]/,"",$2); print $2+0}' "$tmpfile")"
			: "${files_this_dir:=0}"
			rm -f "$tmpfile"

			printf "Directory %s: %'d files would be transferred (SRC=%s → DST=%s)\n" "$base" "$files_this_dir" "$SRC" "$DST"
			SUMMARY+=("Directory $base: $files_this_dir files would be transferred")
			DRY_COPY_FILES=$((DRY_COPY_FILES + files_this_dir))
			COPIED_FILES=$((COPIED_FILES + files_this_dir))
			total_counted_files=$((total_counted_files + files_this_dir))
			any_copied=1
			continue
		fi
		...
		# --- LIVE RUN per tree ---
		mapfile -t RSYNC_ARGS < <(rsync_base_flags)
		RSYNC_ARGS+=(--stats --out-format='%i')

		echo "[LIVE] rsync ${RSYNC_ARGS[*]} \"$SRC/\" \"$DST/\"" >>"$LOG_FILE"

		tmpfile="$(mktemp)"
		files_in_src="$(find "$SRC" -type f 2>/dev/null | wc -l || true)"
		: "${files_in_src:=0}"
		draw_overall "$base" "$files_in_src"
		local files_seen=0
		if rsync "${RSYNC_ARGS[@]}" "$SRC/" "$DST/" 2>&1 | {
			while IFS= read -r line; do
				echo "$line" >>"$tmpfile"
				case "$line" in
				">f"*) files_seen=$((files_seen + 1)); overall_copied=$((overall_copied + 1)); draw_overall "$base" "$files_in_src" ;;
				esac
			done
		}; then
			files_this_dir="$(awk -F': ' '/Number of regular files transferred/ {gsub(/[^0-9]/,"",$2); print $2+0}' "$tmpfile")"
			: "${files_this_dir:=0}"
			# Prefer counted files_seen if stats report 0
			if ((files_this_dir == 0 && files_seen > 0)); then
				files_this_dir="$files_seen"
			fi
			# Suppress per-directory print; keep summary if desired later
			SUMMARY+=("Directory $base: $files_this_dir files copied")
			COPIED_FILES=$((COPIED_FILES + files_this_dir))
			total_counted_files=$((total_counted_files + files_this_dir))
			any_copied=1
			rm -f "$tmpfile"
		else
			echo "✘ Rsync failed for $base ($SRC → $DST)"
			SUMMARY+=("✘ Rsync failed: $base")
			cat "$tmpfile" || true
			rm -f "$tmpfile"
			shopt -u nullglob
			return 1
	...
	# Clear any lingering carriage-return status line
	printf "\r\033[K"
	draw_overall "" 0
	echo
	...
	if ((any_found == 0)); then
		...
		SUMMARY+=("✘ No integer-named directories found under $repo")
		return 0
	fi

	if ((any_copied == 0)); then
	...
	echo
	if [ "$DRY_RUN" = "true" ]; then
		echo "Total files that would be transferred: $(printf "%'d" "$total_counted_files")"
		SUMMARY+=("Total files that would be transferred: $(printf "%'d" "$total_counted_files")")
	else
		banner "✔ Rsync completed (per-tree mapping)" "$GREEN"
		SUMMARY+=("${GREEN}✔ Rsync completed (per-tree) — $(printf "%'d" "$total_counted_files") files${NC}")
	fi
	return 0
}
...
verify_all_refcnt() {
	...
	for d in "$repo"/*; do
		...
		# Count files in source tree for reporting (independent of rsync stats)
		local files_src
		files_src="$(find "$SRC" -type f 2>/dev/null | wc -l || true)"
		: "${files_src:=0}"
		...
		# Count files compared (not transferred) in live verify
		# Report source file count as compared (rsync --stats may include dirs)
		printf "Verify Directory %s: %'d files compared OK\n" "$base" "$files_src"
		SUMMARY+=("Directory $base: $files_src files verified")
		total_verified_files=$((total_verified_files + files_src))
		((verified++))
	done
	shopt -u nullglob
...
main() {
	...
	if [ "$COPY_ONLY" = "true" ]; then
		echo
		banner "=== RUN PREVIEW: COPY-ONLY MODE ===" "$BLUE"
		echo
		banner "✔ MODE: COPY-ONLY (no config/service changes)" "$GREEN"
		...
		[ "$TEST_MODE" = "true" ] && banner "[TEST MODE] mountpoint existence check only" "$YELLOW"

		setup_mountpoint
		echo

		# Populate size and counts
		scan_refcnt_sizes || true
		local planned_files
		planned_files="$(plan_copy_totals)"
		echo "Planned total files to copy: $(printf "%'d" "$planned_files")"
		SUMMARY+=("Planned total files to copy (copy-only): $(printf "%'d" "$planned_files")")

		# Confirm before copying
		read -rp "${RED}Proceed with COPY-ONLY (no config/service changes)? [y/N]: ${NC}" reply
		if [[ "${reply,,}" != "y" && "${reply,,}" != "yes" ]]; then
			echo "✘ Cancelled by user."
			SUMMARY=("${RED}✘ CANCELLED at copy-only confirmation${NC}" "${SUMMARY[@]}")
			print_summary
			exit 0
		fi
		...
		printf "Copy-only duration: %s\n" "$(format_duration "$elapsed")"
		SUMMARY+=("Copy-only duration: $(format_duration "$elapsed")")

		if [ "$VERIFY_CHECKSUM" = "true" ]; then
			echo
			echo "Running verify (checksum=${VERIFY_CHECKSUM}); mismatches may occur if system is active."
	...
	if [ "$DRY_RUN" = "true" ]; then
		...
		# Populate SCAN_TOTAL_BYTES so dry-run space check shows correctly
		scan_refcnt_sizes || true
