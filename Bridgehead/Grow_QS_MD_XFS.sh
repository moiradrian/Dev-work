...
_underlying_base_disks() {
  ...
  if [[ "$type" == "part" ]]; then
    pk="$(_get_parent_kname "/dev/$kname")"; [[ -n "$pk" ]] && echo "$pk" && return 0
  fi
  if [[ "$type" == "disk" || -e "/sys/class/block/$kname/device/rescan" ]]; then echo "$kname"; return 0; fi
...
ask_yes_no(){
  local p="$1"
  [[ "$ASSUME_YES" == "true" ]] && { echo yes; return; }
...
collect_os_info(){
  ...
  kv Kernel     "$(uname -r)"
  if [[ -r /etc/os-release ]]; then . /etc/os-release; kv OS "${PRETTY_NAME:-$NAME}"; else kv OS unknown; fi
...
collect_mount_fs_info(){
  ...
  echo; probe "df -h $(printf %q "$MNT")"
  if have_cmd xfs_info; then echo; echo "-- xfs_info $MNT --" | tee -a "$LOGFILE"; probe "xfs_info $(printf %q "$MNT")"; fi
  get_metadata_usage || true
}
collect_block_graph(){ section "Block Devices (lsblk)"; probe "lsblk -e7 -o NAME,TYPE,SIZE,ROTA,FSTYPE,MOUNTPOINT,PKNAME,MODEL,SERIAL,TRAN -p"; }
...
collect_partition_tables_for_chain(){
  ...
  if have_cmd parted; then
    echo; echo "-- parted -s $parent unit s print --" | tee -a "$LOGFILE"
...
collect_lvm_info_if_any(){
  ...
  have_cmd pvs && { echo "-- pvs (pe_start) --" | tee -a "$LOGFILE"; probe "pvs -o pv_name,vg_name,pv_size,pv_free,pe_start --units m --separator '  ' --noheadings | sed 's/^ *//'"; }
  have_cmd vgs && { echo; echo "-- vgs --" | tee -a "$LOGFILE"; probe "vgs -o vg_name,vg_size,vg_free,pv_count,lv_count --units m"; }
  have_cmd lvs && { echo; echo "-- lvs --" | tee -a "$LOGFILE"; probe "lvs -o lv_name,vg_name,lv_size,lv_path,lv_attr --units m"; }
}
...
compute_package_list(){ case "$PKG_MGR" in
  apt)        PKGS_ALWAYS=(xfsprogs cloud-guest-utils parted util-linux); PKGS_LVM=(lvm2) ;;
  dnf|yum)    PKGS_ALWAYS=(xfsprogs cloud-utils-growpart parted util-linux); PKGS_LVM=(lvm2) ;;
  zypper|apk) PKGS_ALWAYS=(xfsprogs cloud-utils-growpart parted util-linux); PKGS_LVM=(lvm2) ;;
  *)          PKGS_ALWAYS=(); PKGS_LVM=() ;;
esac; }
install_pkgs(){
  local pkgs=("$@"); [[ ${#pkgs[@]} -eq 0 ]] && return 0
  case "$PKG_MGR" in
    apt)    run_mut "DEBIAN_FRONTEND=noninteractive apt-get update -y"; run_mut "DEBIAN_FRONTEND=noninteractive apt-get install -y ${pkgs[*]}" ;;
    dnf)    run_mut "dnf install -y ${pkgs[*]}" ;;
    yum)    run_mut "yum install -y ${pkgs[*]}" ;;
    zypper) run_mut "zypper --non-interactive refresh"; run_mut "zypper --non-interactive install --auto-agree-with-licenses ${pkgs[*]}" ;;
    apk)    run_mut "apk update"; run_mut "apk add ${pkgs[*]}" ;;
...
ensure_precopy_if_needed(){
  ...
  if [[ "$PRECOPY" == "never" ]]; then log "Pre-copy disabled (--precopy=never)."; return 0; fi
  if [[ "$PRECOPY" == "auto" && "$devtype" != "part" ]]; then log "Pre-copy not required (backing device type: ${devtype:-unknown})."; return 0; fi
  ...
  dst_mount=$(pick_main_store); [[ -z "$dst_mount" ]] && die "Could not determine a main storage mount. Specify --main-store <path>."
  kv "Chosen main storage" "$dst_mount"
  avail_bytes=$(bytes_df_avail "$dst_mount")
  local headroom=$(((need_bytes * 11) / 10)) # +10% headroom
  kv "Needed (with headroom)" "$headroom bytes"; kv "Available" "$avail_bytes bytes"

  if (( avail_bytes < headroom )); then
    warn "Not enough space to pre-copy."
    if [[ "$UNSAFE_CONTINUE" == "true" ]]; then warn "Proceeding UNSAFELY without backup due to --unsafe-continue."; return 0; fi
    if [[ "$(ask_yes_no "Proceed UNSAFELY without backup?")" == "yes" ]]; then warn "User accepted UNSAFE continue."; return 0; else die "Aborting; insufficient space for safe pre-copy."; fi
  fi

  # Optional quiesce
  if [[ -n "$QUIESCE_CMD" ]]; then log "Quiescing: $(cmdline $QUIESCE_CMD)"; run_mut "$QUIESCE_CMD" || warn "Quiesce command returned non-zero."; fi

  # Make destination and copy
  local ts; ts="$(date +%Y%m%dT%H%M%S)"; PRECOPY_DIR="${dst_mount%/}/.metadata-precopy-${ts}"
  ...
  if [[ -n "$UNQUIESCE_CMD" ]]; then log "Unquiescing: $(cmdline $UNQUIESCE_CMD)"; run_mut "$UNQUIESCE_CMD" || warn "Unquiesce command returned non-zero."; fi
  ...
  kv "Backup location" "$PRECOPY_DIR"; log "Pre-copy complete."
}
cleanup_precopy_if_requested(){ [[ -n "$PRECOPY_DIR" && "$CLEANUP_PRECOPY" == "true" ]] || return 0; log "Cleaning up backup at $PRECOPY_DIR"; run_mut "rm -rf -- $(printf %q "$PRECOPY_DIR")"; }
...
grow_partition_if_needed(){
  ...
  if [[ "$type" == "part" ]]; then
    ...
    if have_cmd growpart; then
      ...
      # print-only probe with warning filtered
      echo; echo "-- parted -s ${parent} unit s print --" | tee -a "$LOGFILE"
      parted -s "${parent}" unit s print 2>&1 | _filter_parted_tail_warn | tee -a "${LOGFILE}" || true
...
main(){
  ...
  if [[ -z "$MNT" ]]; then discover_target_from_metadata
  else BASE_PATH="${BASE_PATH:-$(get_metadata_path || echo "")}"; [[ -n "$BASE_PATH" ]] && echo -e "\nMetadata location: $BASE_PATH" | tee -a "$LOGFILE"; echo "Target mountpoint: $MNT" | tee -a "$LOGFILE"; fi
  ...
  if [[ "${#__DISKS[@]}" -eq 0 ]]; then
    ...
    if [[ "$any_ok" != "true" ]]; then warn "Per-disk rescan nodes not available; attempting fallback host scan."; _fallback_host_scan; fi
  fi
  ...
  collect_mount_fs_info; collect_block_graph; collect_partition_tables_for_chain; collect_lvm_info_if_any
